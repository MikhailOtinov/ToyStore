...
public class RandomToys {
    ...
    public static void main(String[] args) {
        RandomToys lottery = new RandomToys();
    ...
    private void acceptToyDataFromUser() {
        while (true) {
            try {
                System.out.println(
                        "Введите данные игрушки, разделяя их запятой, без пробела: 'id,Наименование игрушки,Частота выпадения'(Формат 1,Мяч,3):");
                String toyDataString = scanner.nextLine();

                if (toyDataString.trim().isEmpty()) {
                    System.out.println("Ошибка!!!: Введите корретные данные.");
                    continue;
                }
                ...
                if (toyData.length != 3) {
                    System.out.println("Ошибка!!!: Введите все три параметра id,Наименование игрушки,Частота выпадения'(Формат 1,Мяч,3).");
                    continue;
                }
                ...
                if (!checkDuplicate(tempId)) {
                    replaceToyIfExists(new Toys(tempId, tempName, tempFrequency));
                    continue;
                }

                Toys toy = new Toys(tempId, tempName, tempFrequency);
                addToy(toy);
    ...
    private void replaceToyIfExists(Toys newToy) {
        int newToyId = newToy.getId();
        Toys existingToy = findToyById(newToyId);
    ...
    private Toys findToyById(int id) {
        return toySet.stream()
    ...
    private boolean checkDuplicate(int id) {
        return toySet.stream().noneMatch(t -> t.getId() == id);
    }

    private void addToy(Toys toys) {
        toySet.add(toys);
    }
    ...
    private void addToysToQueue() {
        PriorityQueue<Toys> sortedToysQueue = new PriorityQueue<>(new ToyComparator());
    ...
    public class ToyComparator implements Comparator<Toys> {
        @Override
        public int compare(Toys toy1, Toys toy2) {
            return Integer.compare(toy2.getFrequency(), toy1.getFrequency());
        }
    }

    public void displayCurrentCollection() {
        DisplaySet.displaySet(toySet);
    }
